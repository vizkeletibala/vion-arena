...
Canonical Milestone 1 scene path: `Assets/Game/Scenes/PrototypeArena.unity`. The scene is a hand-authored greybox slice that wires together `VitrialPlayer`, `StarterRifle`, `GreyboxChaser`, loot drops, pickup/inventory UI, and a scene restart loop. See `docs/vitrial-unity.md` for folder conventions, Milestone 3 client/server seam prep, and handoff notes, and `docs/vitrial-automation.md` for the EC2/Jenkins/server automation boundary.
