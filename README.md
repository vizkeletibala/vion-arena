...
Canonical Milestone 1 scene path: `Assets/Game/Scenes/PrototypeArena.unity`. The scene is a hand-authored greybox slice that wires together `VitrialPlayer`, `StarterRifle`, `GreyboxChaser`, loot drops, pickup/inventory UI, and a scene restart loop. See `docs/vitrial-unity.md` for folder conventions, Milestone 3 client/server seam prep, `docs/vitrial-automation.md` for the EC2/Jenkins/server automation boundary, and `docs/vitrial-ci-cd-strategy.md` for Andrew's recommended near-term/manual and medium-term automated Unity CI path.

Runtime server prep is intentionally lightweight: `GameModeDefinition` now owns authority mode plus host/port/secure-transport/auto-connect settings, `MatchBootstrap` exposes the connection config, and `Assets/Game/Scripts/Networking/` contains transport-agnostic authority seams. The default remains `LocalPrototype`, so the greybox slice does not attempt a network connection until a future task opts into `ServerAuthoritative` and chooses a real transport package.
